...
#[test]
fn test_lev_distance_closest() {
    let a = "TA";
    let b_vec = vec!["TCA", "CAG"];

    let b_tca = "TCA";
    let b_cag = "CAG";

    assert_eq!(lev_distance(a, b_tca), 1);
    assert_eq!(lev_distance(a, b_cag), 2);

    let closest_choice = closest(a, b_vec.iter(), |c| c);
    assert_eq!(closest_choice, Some(&"TCA"));
}
